...
- Node.js 24.15.0, Rust 1.97.1, Git 2.54.0.windows.1;
...
The Weavatrix and atomwrite matrices requested 1, 2, 4, and 8 workers. Git has
no worker control, so its worker axis is one explicit `null` configuration.
...
| git apply 2.54.0 | 280 | 240 | 4 dry-run and 4 non-durable-apply configurations | none |

All 560 atomwrite transaction rows exited successfully and produced every
expected content hash. All 560 also left timestamped `.bak` files, failed the
artifact-cleanup gate, and emitted a Windows `fsync_file best-effort` warning.
They therefore have no publishable latency statistics and are not treated as
equivalent to the Weavatrix recoverable-durable contract.

Git passed every content and cleanup gate for its declared modes, but documents
no worktree fsync, journal, restart recovery, full-file CAS, or worker control.
Its apply results remain in the separate non-durable class.
...
patterns, and Git uses unified-diff context.

| Files | Weavatrix workers | Weavatrix p50 / p95 ms | atomwrite workers | atomwrite p50 / p95 ms | Git p50 / p95 ms | Weavatrix p50 advantage vs atomwrite / Git |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
...
For context only, Git's separate non-durable 64-file apply result was
213.10 ms p50 and 270.52 ms p95. It must not be compared as if it provided the
same durability or recovery contract.
