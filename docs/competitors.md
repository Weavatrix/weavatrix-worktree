...
| [`atomwrite` 0.1.36](https://docs.rs/crate/atomwrite/0.1.36) | Direct agent-oriented competitor with write/replace/delete/move batch operations, backups, and transaction mode. | Standalone write exposes `--expect-checksum`; the audited batch input schema has no expected-checksum field, so this benchmark class is not full-file CAS equivalent. | Implements a workspace jail and documents link, special-file, and hostile-path checks. | Batch exposes explicit `--threads`; worker use is recorded rather than inferred. | Transaction mode uses compensating backups/removal/reverse moves after an error. This is rollback, not atomic multi-path visibility or the Weavatrix persisted restart-recovery contract. | Individual operations advertise atomic writes and synchronization, but the batch contract is reported separately from recoverable journaled commit. Its successful transaction path retains rollback `.bak` snapshots for pre-existing paths despite `--no-backup`, per-operation `backup:false`, and default `keep-backup:false`; durable results are audit-only and fail cleanup. |

### Resource-operation support used by the harness

| Adapter | Modify | Create | Delete | Rename | Published comparison class |
| --- | --- | --- | --- | --- | --- |
| Weavatrix | exact edits + full SHA-256 | `MustNotExist` | full SHA-256 | source SHA-256 + absent destination | recoverable full-CAS batch |
| atomwrite 0.1.36 batch | exact replacement pattern | `write` | `delete` | `move` | compensating transaction without batch full-file CAS |
| system Git `apply` | unified-diff context | new-file patch | deleted-file patch | extended rename patch | whole-patch preflight / non-durable apply |
| LSP `WorkspaceEdit` | text edit | yes | yes | yes | protocol only; no filesystem latency class |
| atomic-write-file 0.3.0 | replacement primitive | one file | no | no | single-file primitive |
| textum / rustfix | text only | no | no | no | not a resource-operation competitor |
...
4. Transaction mode [unconditionally snapshots pre-existing paths](https://github.com/danilo-aguiar-br/atomwrite/blob/c5aedb669d38938848ce053f1476914429b5c554/src/commands/batch/run.inc.rs#L100-L129)
   before mutation. Its [successful return path](https://github.com/danilo-aguiar-br/atomwrite/blob/c5aedb669d38938848ce053f1476914429b5c554/src/commands/batch/run.inc.rs#L377-L397)
   writes the summary and returns without removing those snapshots. The batch
   command exposes no transaction-snapshot cleanup switch, so deleting them in
   a benchmark wrapper would measure a contract the competitor does not expose.
